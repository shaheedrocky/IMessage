...
export default function ChatHeader() {
  ...
  const isTyping = activeConversation ? typingStates[activeConversation.id] : false;
    ...
    <header className="sticky top-0 z-10 flex shrink-0 flex-wrap items-center gap-1 border-b border-border px-1.5 py-1.5 sm:gap-2 sm:px-2 sm:py-2">
      ...
      {activeConversation ? (
        <>
          ...
          <div className="flex-1 text-start ml-1 sm:text-left">
            <p className="truncate text-[15px] font-semibold leading-tight">
              {activeConversation.peer.name}
            </p>
            <p className="truncate text-xs text-muted">
              {isTyping ? (
                <span className="font-medium text-accent animate-pulse">typing...</span>
